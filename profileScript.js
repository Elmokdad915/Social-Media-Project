...
function getPosts() {
    ...
    .then((response) => {
        ...
        for (post of posts) {
            ...
            if (isMyPost) {
            editBtnContent = `
            <button class="btn btn-outline-danger" style="margin-left: 5px; float: right;" onclick="deletePostBtnClicked('${encodeURIComponent(
                JSON.stringify(post)
            )}')">Delete</button>
            ...
            let content = `
                ...
                <div class="card-header"  >
                    ...
                    src="${author.profile_image}" onerror= "this.src = './profile_pics/user-pic.png'"
                    alt=""
            ...
            let tagsContent = `
            <button class=" btn-sm rounded-5 text-bg-secondary">
                ${tag.name}
            </button>
        `;
